...
#ifdef _WIN32
string TCPClient::SendWin(string hostname, string portNumber, string message) {
  ...
  status = getaddrinfo(hostname.c_str(), portNumber.c_str(),
  ...
  socketfd = socket(host_info_first->ai_family, host_info_first->ai_socktype,
                    host_info_first->ai_protocol);
  ...
  status = connect(socketfd, host_info_first->ai_addr,
                   static_cast<int>(host_info_first->ai_addrlen));
...
string TCPClient::SendOverSSL(string hostname, string portNumber,
                              string message) {
